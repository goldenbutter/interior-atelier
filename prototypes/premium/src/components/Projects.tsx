...
type Project = (typeof copyNo.projects.list)[number];

// Single project card. Desaturated by default; colourises on hover (desktop)
// or on tap (touch). Touch activation auto-reverts after 2s so the wall of
// projects keeps reading as one editorial composition.
function ProjectCard({
  ...
  const isTall = "tall" in project && project.tall === true;
  const isWide = "wide" in project && project.wide === true;
  ...
  return (
    <motion.a
      href={`#prosjekt-${project.slug}`}
      onClick={handleTap}
      initial={{ opacity: 0, y: 50 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: "-80px" }}
      transition={{ duration: 1, delay: (index % 2) * 0.12, ease }}
      className={`group block ${
        isWide ? "md:col-span-2" : index % 2 === 1 ? "md:mt-20" : ""
      }`}
    >
      <div
      ...
      >
        <div className="bezel-inner relative h-full w-full overflow-hidden rounded-[calc(1.75rem-0.375rem)] bg-cream">
          <Image
            ...
            className={
              active
                ? "object-cover grayscale-0 saturate-100 scale-[1.05] transition-all duration-[1600ms] ease-[cubic-bezier(0.22,1,0.36,1)]"
                : "object-cover grayscale-[0.85] saturate-[0.7] transition-all duration-[1600ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:grayscale-0 group-hover:saturate-100 group-hover:scale-[1.05]"
            }
          />
          <div className="absolute left-5 top-5 flex items-center gap-3">
            <span className="inline-flex h-9 items-center rounded-full bg-bone/85 px-4 font-mono text-[10px] uppercase tracking-[0.22em] text-charcoal backdrop-blur-md ring-1 ring-charcoal/5">
              {project.no} <span className="mx-1.5 text-stone/60">/</span>{" "}
      ...
      <div className="mt-8 flex items-start justify-between gap-6">
        <div>
          ...
          <p className="eyebrow mt-3">{project.location}</p>
        </div>
